...
//   6. Install              → Assets/Plugins/AntiGravitySkills/<name>/
...
export async function addSkill(skillName, opts = {}) {
    ...
    // ── 1. Scan Unity project ──────────────────────────────────────────────────
    let spin = spinner('Scanning Unity project...');
    const ctx = scanProject();
    spin.stop();

    if (!ctx) {
        log.error('No Unity project found. Run this command from inside a Unity project folder.');
    ...
    // ── 5. Already installed? ──────────────────────────────────────────────────
    if (isAlreadyInstalled(ctx.projectRoot, meta.name) && !opts.force) {
    ...
    // ── 6. Confirm install ─────────────────────────────────────────────────────
    log.blank();
    const destPlaceholder = `Assets/Plugins/AntiGravitySkills/${meta.name}/`;
    log.info(`Ready to install to: ${destPlaceholder}`);
    ...
    if (frontmatter?.tdd_first === false) {
        log.blank();
        log.warn('This is a Legacy skill (tdd_first: false). Review generated code carefully.');
    }
